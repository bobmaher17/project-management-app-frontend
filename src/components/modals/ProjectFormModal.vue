  ...
  <div class="fixed inset-0 bg-black/60 flex justify-center items-center z-50 p-4">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-lg">
      <header class="p-5 border-b flex justify-between items-center">
        <h3 class="font-semibold text-lg text-slate-800">{{ isEditing ? 'Ubah Proyek' : 'Tambah Proyek Baru' }}</h3>
      ...
      <form @submit.prevent="submitForm" class="p-6 space-y-4 bg-slate-50">
        <div>
          ...
          <input type="text" v-model="form.projectName" required class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm p-3">
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Bank</label>
          <input type="text" v-model="form.bank" required class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm p-3">
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Status Proyek</label>
          <select v-model="form.projectStatus" required class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm p-3">
...
const form = ref<Partial<Project>>({ projectName: '', bank: '', projectStatus: 'Not Yet' });
const isEditing = computed(() => !!props.project);
onMounted(() => {
  if (isEditing.value) form.value = JSON.parse(JSON.stringify(props.project));
});
